...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    float nms=.3;
    if(!filename) {
        printf("Enter Directory Path (ex. data/samples): ");
        fflush(stdout);
        scanf("%s", buff2);
        directory = buff2;
        FILE *fw = fopen("results/bboxes.txt", "w");
        fprintf(fw, "Shows Class, Probability, and Bounding Box coordinates\n");
        fclose(fw);
        FILE *f = fopen("data/path_list.txt", "w+");
        if((dir = opendir (directory)) != NULL) {
            //save all files into a text file
        ...
        fclose(f);
        FILE *f2 = fopen("data/filenames.txt", "w+");
        if((dir = opendir (directory)) != NULL) {
            //save all files into a text file
            while((ent = readdir (dir)) != NULL) {
                if(ent->d_type == DT_REG) {
    ...
    FILE *fr = fopen("data/path_list.txt", "r");
    FILE *fr2 = fopen("data/filenames.txt", "r");
    while(1){
        if(filename){
            strncpy(input, filename, 256);
            strncpy(file, "predictions", 256);
        } else {
            //printf("Enter Image Path: ");
            //fflush(stdout);
            //input = fgets(input, 256, stdin);
            getline(&input, &len, fr);
            getline(&file, &len2, fr2);
            strtok(input, "\n");
        }
        ...
        draw_detections(im, input, l.w*l.h*l.n, thresh, boxes, probs, masks, names, alphabet, l.classes);
        if(outfile){
